...
function drawFlowchart($xml,selector,options){
    ...
    if(options.viewDesc){
        ...
        $output.append('<div class="desc" style="margin:0;border-bottom:1px solid #aaa;text-align:left">'+desc+'</div>'+
              '<div class="info" style="padding:.5em 0;font-size:50px;text-align:left">'+
              '<a>Title: </a>'+
              '<span class="title">'+title+'</span>'+
              '<a> Note you can save this by pressing Ctrl+p</a></div>'
            );
    }
    $output.append('<svg class="flowchart" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">'+
      ...
      '  .start{stroke:#000;stroke-width:1px;fill:#a060c0}'+
      '  .end{stroke:#000;stroke-width:1px;fill:#a060c0}'+
      '  .declare{stroke:#000;stroke-width:1px;fill:#f0c020}'+
      '  .assig{stroke:#000;stroke-width:1px;fill:#f0c020}'+
      '  .inp{stroke:#000;stroke-width:1px;fill:#2080e0}'+
      '  .out{stroke:#000;stroke-width:1px;fill:#30c090}'+
      '  .if{stroke:#000;stroke-width:1px;fill:#e03040}'+
      '  .loop{stroke:#000;stroke-width:1px;fill:#f06020}'+
    ...
    function drawStart(content){
        ...
        var s = '<g class="block" transform="translate(0,'+flow.Y+')">'+
                '  <g class="symbol">'+
                '    <ellipse class="start" cx="0" cy="0" rx="'+l+'" ry="20"/>'+
    ...
    function drawEnd(content){
        ...
        var s = '<g class="block" transform="translate(0,'+flow.Y+')">'+
                drawArrow()+
                '  <g class="symbol" transform="translate(0,'+(options.aH+options.aT+20)+')">'+
                '    <ellipse class="end" cx="0" cy="0" rx="'+l+'" ry="20"/>'+
                '    <text x="0" y="4">'+content+'</text>'+
                '  </g>'+
                '</g>';
    ...
    function drawDeclare(content){
        ...
        var s = '<g class="symbol" transform="translate(0,'+flow.Y+')">'+
                drawArrow()+
                '  <g class="assign" transform="translate('+(-l/2)+','+(options.aH+options.aT)+')">'+
                '    <rect class="declare" width="'+l+'" height="36"/>'+
    ...
    function drawAssign(content){
        ...
        var s = '<g class="symbol" transform="translate(0,'+flow.Y+')">'+
                drawArrow()+
                '  <g class="assign" transform="translate('+(-l/2)+','+(options.aH+options.aT)+')">'+
                '    <rect class="assig" width="'+(l)+'" height="36"/>'+
    ...
    function drawInput(content){
        ...
        var s = '<g class="symbol" transform="translate(0,'+flow.Y+')">'+
                ...
                '    <polygon class="inp" points="0,0 -20,36 '+l+',36 '+(20+l)+',0"/>'+
                '    <text x="'+(l/2)+'" y="23">'+content+'</text>'+
                '  </g>'+
                '</g>';
        flow.Y += 36+options.aH+options.aT;
    ...
    function drawOutput(content){
        ...
        var s = '<g class="symbol" transform="translate(0,'+flow.Y+')">'+
                ...
                '    <polygon class="out" points="0,0 -20,36 '+l+',36 '+(20+l)+',0"/>'+
                '    <text x="'+(l/2)+'" y="23">'+content+'</text>'+
                '  </g>'+
                '</g>';
        flow.Y += 36+options.aH+options.aT;
    ...
    function drawMore(){
        ...
        var s = '<g class="loop" transform="translate(0,'+flow.Y+')">'+
                drawArrow()+
    ...
    function drawSelection(condition,$branchTrue,$branchFalse){
        ...
        var s = '<g class="symbol" transform="translate(0,'+oldY+')">'+
                drawArrow()+
                '  <g class="selection" transform="translate(0,'+(options.aH+options.aT)+')">'+
                '    <g class="condition">'+
                '      <polygon class="if" points="0,0 '+halfCondition+','+romH+' 0,'+(romH*2)+' '+(-halfCondition)+','+(romH)+'"/>'+
                        condition+
                '    </g>'+
                '    <g class="arrow-right">'+
    ...
    function drawPreIteraction(condition,$content){
        ...
        var s = '<g class="symbol" transform="translate(0,'+flow.Y+')">'+drawArrow();
        var oldY = flow.Y;
        flow.Y = 0;
        ...
        s +=    '  <g class="preIteraction" transform="translate(0,'+(options.aH+options.aT)+')">'+
                '    <g class="condition">'+
                '      <polygon class="loop" points="0,0 '+condW2+',0 '+(condW2+15)+','+romH+' '+condW2+','+(romH*2)+' '+(-condW2)+','+(romH*2)+' '+(-condW2-15)+','+(romH)+' '+(-condW2)+',0"/>'+
                        condition+
                '    </g>';
    ...
    function drawPostIteraction(condition,$content){
        ...
        var condW2 = contidionWidth/2;
        var s = '<g class="symbol" transform="translate(0,'+flow.Y+')"><line class="line" x1="0" y1="0" x2="0" y2="'+(options.aH+options.aT)+'"/><circle class="loop" cx="0" cy="'+(options.aH+options.aT)+'" r="4"/>';
        var oldY = flow.Y;
        flow.Y = 0;
        var contentDraw = ($content.children().length>0) ? drawSequence($content) : drawEmptyBlock();
        var contentWidth = calcBlockWidth(contentDraw);
